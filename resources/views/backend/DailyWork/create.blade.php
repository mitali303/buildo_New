...
<form action="{{ !empty($dailyWork) ? route('DailyWork.update') : route('DailyWork.store') }}"
      method="POST">
...
<input type="date"
       ...
       value="{{ old('date',$dailyWork->date ?? date('Y-m-d')) }}">
...
<select name="ClientID"  id="scheme"
...
{{ old('ClientID',$dailyWork->ClientID ?? '') == $scheme->ID ? 'selected':'' }}
...
<input type="hidden"
       name="sitename"
       id="sitename"
       value="{{ old('sitename',$dailyWork->sitename ?? '') }}">
...
<textarea name="workdone"
          rows="5"
          class="form-control @error('workdone') is-invalid @enderror"
