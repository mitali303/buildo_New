@php
    ...
    $payamount   = old('payamount.' . ($loop->index ?? null), $isEdit ? $row['payamount'] : ($payamount ?? 0));
    $amtVal      = old('amt.' . ($loop->index ?? null),       $isEdit ? $row['amt']       : ($amt ?? 0));
    $balanceVal  = old('balance.' . ($loop->index ?? null),   $isEdit ? $row['balance']   : ($balance ?? ''));
...
<tr class="invoice-row" style="font-size:13px;">
    ...
    {{-- Amount --}}
    <td>
        <input type="text" name="amt[]" class="form-control amt" value="{{ $amtVal }}" {{ $isChecked ? '' : 'disabled' }}oninput="calculateBalance(this)">
    </td>
    {{-- Balance --}}
    <td>
