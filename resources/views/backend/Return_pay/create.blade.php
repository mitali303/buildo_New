            ...
            <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4" id="exp-type-container">
                            ...
                            <select name="Exp_type" id="Exp_type"
                                ...
                                <option value="">Select</option>
                                 <option value="OTHER" style="color: #fff; background-color: #198754; font-weight: bold;">
                                    ➕ ADD NEW
                                </option>
                                ...
                                <option value="">Select</option>
                                    @foreach($contractors as $contractor)
                                        <option value="{{ $contractor->ID }}"
                                            {{ old('contractor',  $payment->conID ?? '') == $contractor->ID ? 'selected' : '' }}>
                                            {{ $contractor->Name }}
                                        </option>
                                    @endforeach
                            </select>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th style="width: 12%;">Amount Receive</th>
                            <th style="width: 15%;">Cheque No / Transaction ID</th>
                            <th style="width: 12%; display:none;" id="banktitlee">Bank Charges</th>  
                    ...
                    <tbody>
                        <tr>
                            ...
                            {{-- Bank Charges --}}
                            <td id="bankvaluee" style="display:none;">
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.length > 0) {
                    ...
                    // 🔥 SELECT OLD INVOICE (EDIT MODE)
                    // 🔥 SELECT OLD INVOICE (EDIT MODE)
                    const oldInvoice = "{{ $payment->workorderID ?? '' }}";
                    if (oldInvoice) {
                        invoiceChoices.setChoiceByValue(oldInvoice);
    ...
    $('#supplier').on('change', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // 🔥 SELECT OLD ORDER (EDIT MODE)
                // 🔥 SELECT OLD ORDER (EDIT MODE)
                const oldOrder = "{{ $payment->Invno ?? '' }}";
                if (oldOrder) {
                    orderChoices.setChoiceByValue(oldOrder);

                    // 🔥 FORCE pending + table AJAX
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const blocks = {
        ...
        contractor : '#contractor-div',
        supplier   : '#supplier-div',
        order      : '#order-container',
        pending    : '#pending_amount',
        orderTable : '#order-table-container'
    };

    function hideAll() {
        Object.values(blocks).forEach(sel => $(sel).addClass('d-none'));
    }
    ...
    function handleExpenseType(expType) {
        ...
        if (expType === 'salary' || expType === 'advanced salary') {
            $(blocks.employee).removeClass('d-none');
            ...
            $(blocks.contractor).removeClass('d-none');
        ...
        else if (expType === 'return / rejected material') {
            $(blocks.supplier).removeClass('d-none');
            ...
            loadMaterialTransferPending();
        }
    }

    function loadMaterialTransferPending() {
        let schemeId = $('#Destination').val();
        if (!schemeId) return;

        $.post("{{ route('ajax.getMaterialTransferBalance') }}", {
            scheme_id: schemeId,
            _token: "{{ csrf_token() }}"
        }, function (res) {
            $('#pending').val(res.pending);
            $(blocks.pending).removeClass('d-none');
        });
    }
...
@if(isset($payment) && $payment->IncomeType === 'Contractor')
<script>
document.addEventListener('DOMContentLoaded', function () {

    const contractorId = "{{ $payment->conID }}";
...
@endif
@if(isset($payment) && $payment->IncomeType === 'Return / Rejected Material')
<script>
document.addEventListener('DOMContentLoaded', function () {

    const supplierId = "{{ $payment->vendor }}";
