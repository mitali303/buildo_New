...
@section('title', !empty($transfer) ? 'Edit Material Consumption' : 'Create Material Consumption')
<style>
                ...
                <form action="{{ !empty($transfer) ? route('Material_Consumption.update', $transfer->ID) : route('Material_Consumption.store') }}" method="POST">
                    ...
                    <div class="row">
                               ...
                               <div class="input-group flatpickr-container">
                                <input type="text" name="Date"
                                    ...
                                    placeholder="Select date"
                                     value="{{ old('Date', isset($transfer->Date) ? \Carbon\Carbon::parse($transfer->Date)->format('d-m-Y') : now()->format('d-m-Y')) }}"
                                >
                            </div>
                       ...
                       <div class="col-md-4" id="scheme">
                            ...
                            <select name="scheme" id="scheme"
                                    class="form-control choices-single-scheme"
                                ...
                                <table class="table table-bordered table-striped" id="materialTable" style="font-size:13px;">
                                    ...
                                    @for($i = 0; $i < $rowCount; $i++)
                                        ...
                                        @php
                                            ...
                                            // ✅ Material NAME via ID
                                            $materialName = old("material_group.$i")
                                                ?? ($hasProduct ? ($materialIdToName[$products[$i]->material] ?? '') : '');
                                            ...
                                            // ✅ Type via ID
                                            $type = old("type.$i")
                                                ?? ($hasProduct ? ($materialIdToType[$products[$i]->material] ?? '') : '');

                                            $unit = old("unit.$i")
                                                ?? ($hasProduct ? $products[$i]->Unit : '');
                                        ...
                                        <tr style="font-size:13px;">
                                            <td>
                                                ...
                                                <input type="hidden" name="material_id[]"
                                                    class="material_id"
                                                    value="{{ old('material_id.'.$i, $hasProduct ? $products[$i]->material  : '') }}">
                                            </td>

                                            <td>
                                                <select name="type[]" class="form-control mat-type" data-selected="{{ $type }}">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <a href="{{ route('Material_Consumption') }}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('addRow').addEventListener('click', function () {
        fetch('{{ route('Material_Consumption.addRow') }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        .then(html => {
            ...
            $('#materialTable tbody tr:last .choices-single-material').select2({
...
function chk_qty(el) {
    ...
    .then(mat => {
        ...
        // 🔹 STEP 2: Get stock
        return fetch("{{ route('Material_Consumption.getStock') }}", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-TOKEN": "{{ csrf_token() }}"
